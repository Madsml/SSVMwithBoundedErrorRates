...
tune_klr_ova = function(x_train,y_train,x_tune,y_tune,x_threshold=x_tune,y_threshold=y_tune,x_test,y_test,r,r_match,lambda_pool = 10^(-3:3),kernel='radial',rho_pool = 1,degree_pool=2) {
  ...
  for (jj in 1:class_number) {
    ...
    score_test = -(K_test%*%model_best[[jj]]$v+model_best[[jj]]$b)
