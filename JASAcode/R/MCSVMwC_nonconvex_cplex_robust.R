...
# Train
#############################################################################################################################
mcsvmwc.qp.lp=function(X,Y,initial=NULL,k=NULL,weight=NULL,lambda,r,kernel='linear',jitter=1e-10,rho=1,degree=1,max_iteration=20,...){
  n = nrow(X)
  p = ncol(X)
  # get the number of classes
  ...
  # then do quadratic programming via Cplex: min_{b} t(d)x + 1/2t(x)Dx, s.t. A x <= x_0 and lb <= x <= ub
  ## quadratic(D) part
  ### kernel part
  if (kernel == 'linear') {
    K=LinearKern(X,X)
  ...
  # make D_OnlyAlpha exactly symmetric
#  for(i in (2:(k*n))) {
#    for (j in (1:(i-1))) {
...
#  }
  ...
  A_3 = matrix(0,nrow=n,ncol=(n*k))
  A_theta = matrix(0,nrow=n,ncol=k)
  for (i in 1:n) {
    A_3[i,(((Y[i]-1)*n)+i)] = -1
    A_theta[i,Y[i]] = weight[i]
  }
  ### last constraint
  A_4 = -t(Y_vec)
  ### get A
  A = matrix(0,nrow = (n+k),ncol = ((n*k)+k))
  A[1:(k-1),1:(n*k)] = t(A_1)
  A[((k):(n+k-1)),(1:(n*k))] = A_3
  A[((k):(n+k-1)),(((n*k)+1):((n*k)+k))] = A_theta
  A[(n+k),(1:(n*k))] = A_4
  # start the d.v. loop
  indicator = 0
  while((indicator < max_iteration)) {
    ...
    b_0 = rep(0,(n+k))
    b_0[1:(k-1)] = -c_for_b
    b_0[(n+k)] = c_for_epsilon
    # get upper bounds
    upper_mat = matrix(lambda,nrow = n,ncol = k)
    for (i in 1:n) {
      upper_mat[i,Y[i]] = Inf
    }
    upper_vec = vectorize(upper_mat)
    # solver
    require(Rcplex)
#    start_time = proc.time()
    res = Rcplex(cvec = -d, Amat = A, bvec = b_0, Qmat = D, ub = c(upper_vec,rep(Inf,k)), control=list(trace=0),objsense = "min", sense = c(rep('E',(k-1)),rep('G',(n+1))))
...
# tuning
#############################################################################################################################
mcsvmwc_tune = function(x_train,y_train,x_tune,y_tune,r,lambdapool=10^((-8:8)/2),kernel='linear',max.iteration = 20,max.violation = 1,degree_pool=1,rho_pool=1,small_tol = 1e-6,...) {
  n = nrow(x_train)
  final_ambiguity = Inf
    ...
    for (rho in rho_pool) {
      ...
      for (i in 1:length(lambdapool)) {
        ...
        while ((max(abs(weight-w_0))>0.01 && indicator <= max.iteration) || (indicator == 1)) {
          ...
          w_0 = weight
          model=mcsvmwc.qp.lp(X=x_train,Y=y_train,weight=w_0,lambda=lambdapool[i],r=r,kernel=kernel,degree=degree,rho=rho)
          weight=model$newweight
          indicator=indicator+1
        }
        pre = predict.mcsvmwc(model,x_tune,y_tune)
        temp = pre$'ambiguity proportion'
        tempcon = pre$control
        print(c(temp,tempcon))
        if (temp < (ambiguity+small_tol)&& max(tempcon-r) < small_tol) {
          ambiguity = temp
      ...
      for (ii in 1:length(newpool)) {
        ...
        while ((max(abs(weight-w_0))>0.01 && indicator <= max.iteration) || (indicator == 1)) {
          print(paste('iteration for weight = ',indicator))
          w_0 = weight
          model=mcsvmwc.qp.lp(X=x_train,Y=y_train,weight=w_0,lambda=newpool[ii],r=r,kernel=kernel,degree=degree,rho=rho,...)
          weight=model$newweight
          indicator=indicator+1
        }
        pre = predict.mcsvmwc(model,x_tune,y_tune)
      ...
      if (ambiguity < (final_ambiguity+small_tol)) {
...
# robust tuning
#############################################################################################################################
mcsvmwc_tune_robust = function(x_train,y_train,x_tune,y_tune,x_threshold=x_tune,y_threshold=y_tune,r,lambdapool=10^((-8:8)/2),kernel='linear',max.iteration = 20,max.violation = 1,degree_pool=1,rho_pool=1,small_tol = 1e-6,...) {
  n = nrow(x_train)
  index_list = list()
  allowance = rep(1,length(r))
  for (i in 1:length(r)) {
    index_list = c(index_list,list(which(y_threshold==i)))
    allowance[i] = floor(r[i] * length(which(y_threshold==i)))
  }
  final_ambiguity = Inf
  for (degree in degree_pool) {
    for (rho in rho_pool) {
      ...
      for (i in 1:length(lambdapool)) {
        ...
        pre_original = predict.mcsvmwc(model,x_threshold,y_threshold)
        # define a new kind of threshold
        score_threshold = pre_original$score
        thresholds = rep(0,length(r))
        for (jj in 1:length(r)) {
          thresholds[jj] = sort(score_threshold[index_list[[jj]],jj],decreasing = FALSE)[allowance[jj]]
        }
