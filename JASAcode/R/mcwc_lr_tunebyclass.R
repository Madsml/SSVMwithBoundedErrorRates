...
tune_lr_tbc = function(x_train,y_train,x_tune,y_tune,x_threshold=x_tune,y_threshold=y_tune,x_test,y_test,r,r_match,nlambda = 100,alpha_pool = 0) {
  ...
  for (ii in 1:class_number) {
    ...
    for (i in 1:length(alpha_pool)) {
      ...
      model = glmnet(x = x_train,y = y_train_temp,family='binomial',alpha=alpha,nlambda = nlambda)
      coefficient_matrix = matrix(model$beta,nrow = p,ncol = dim(model$beta)[2])
      intercept = model$a0
      for (j in 1:dim(model$beta)[2]) {
        beta = coefficient_matrix[,j]
        b = intercept[j]
        # scores for threshold set
        score_threshold = x_threshold%*%beta+b 
    ...
    score_test = x_test%*%beta_best + b_best
  ...
  rownames(simulation) = c('Logit_tbc')
