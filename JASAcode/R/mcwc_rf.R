...
tune_rf = function(x_train,y_train,x_tune,y_tune,x_threshold=x_tune,y_threshold=y_tune,x_test,y_test,r,r_match,ntree_pool = seq(50,500,by=50),mtry_pool=c(0.3,0.4,0.5,0.6,0.7,0.8)) {
  ...
  for (i in 1:length(ntree_pool)) {
    for (j in 1:length(mtry_pool)) {
      ...
      model = randomForest(x=x_train,y=as.factor(y_train),ntree=ntree,mtry=(mtry*ncol(x_train)))
      prob_tune = predict(object = model,newdata = x_tune,type = 'prob')
      prob_threshold = predict(object = model,newdata = x_threshold,type = 'prob')
  ...
  prob_test = predict(object = model_best,newdata = x_test,type = 'prob')
  ...
  rownames(simulation) = c('ranForest')
