...
tune_svm_tbc = function(x_train,y_train,x_tune,y_tune,x_threshold=x_tune,y_threshold=y_tune,x_test,y_test,r,r_match,cost_pool = 10^(-3:3),kernel='radial',gamma_pool = 1,degree_pool=2) {
...
#         print(c(i,k))
          ...
          score_threshold = attr(predict(model,x_threshold,probability = TRUE),'probabilities')[,2]
          ...
          score_tune = attr(predict(model,x_tune,probability = TRUE),'probabilities')[,2]
  ...
  score_test = attr(predict(model_best,x_test,probability = TRUE),'probabilities')[,2]
  ...
  colnames(simulation) = c('ambiguity_robust',robust_name,'ambiguity_adjust',adjust_name)
  rownames(simulation) = c('SVM_tbc')
